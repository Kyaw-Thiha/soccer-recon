...
class SNV3Dataset(Dataset):
    def __init__(
        ...
        self.resize = torchvision.transforms.Resize(
            (resolution[1], resolution[0]), antialias=True
        )
        ...
        for game in tqdm(self.list_games):
            # Check for v3D labels first if use_v3d is True
            labels_path = os.path.join(self.path, game, "Labels-v3D.json" if use_v3d else "Labels-v3.json")

            # Fall back to v3 if v3D doesn't exist
            if use_v3d and not os.path.exists(labels_path):
                print(f"Warning: Labels-v3D.json not found for {game}, falling back to Labels-v3.json")
                labels_path = os.path.join(self.path, game, "Labels-v3.json")

            self.metadata.append(json.load(open(labels_path)))
        ...
        for annotations in tqdm(self.metadata):
            ...
            for action_name in annotations["GameMetadata"]["list_actions"]:
                ...
                img_list = [action_name] + annotations["actions"][action_name][
                    "linked_replays"
                ]
                ...
                zipfilepath = os.path.join(
                    self.path, annotations["GameMetadata"]["UrlLocal"], "Frames-v3.zip"
                )
                ...
                for i, img in enumerate(img_list):
                    ...
                    # Try multiple possible frame locations
                    base_path = os.path.join(self.path, annotations["GameMetadata"]["UrlLocal"])

                    # Check in order: extracted frames/, v3_frames/, root directory
                    possible_paths = [
                        os.path.join(base_path, "frames", img + ".png"),
                        os.path.join(base_path, "v3_frames", img),
                        os.path.join(base_path, img + ".png"),
                    ]
                    ...
                    # Default to first option if preloading isn't enabled
                    if filepath is None:
                        filepath = possible_paths[0]
                    if self.preload_images:
                        with torch.no_grad():
                            if self.zipped_images:
                                # Images in zip should be named like "0.png", "1_1.png"
                                img_name = img + ".png" if not img.endswith(".png") else img
                                imginfo = zippedFrames.open(img_name)
                                data_tmp["image"] = self.resize(
                                    transforms.ToTensor()(Image.open(imginfo)) * 255
                                )
                            else:
                                data_tmp["image"] = self.resize(
                    ...
                    # Store image name with .png extension
                    img_name = img + ".png" if not img.endswith(".png") else img
                    data_tmp["imagefilepath"] = img_name
                    data_tmp["filepath"] = filepath

                    data_tmp["bboxes"], ID_tmp = self.format_bboxes(
                        annotations[img_type][img]["bboxes"],
                        annotations[img_type][img]["imageMetadata"],
                    )

                    data_tmp["lines"] = self.format_lines(
                        annotations[img_type][img]["lines"],
                        annotations[img_type][img]["imageMetadata"],
                    )
    ...
    def format_bboxes(self, bboxes, image_metadata):
        ...
        for i, bbox in enumerate(bboxes):
            if bbox["class"] is not None:
                ...
                tmp_data[2] = (
                    abs(bbox["points"]["x2"] - bbox["points"]["x1"])
                    / image_metadata["width"]
                )
                tmp_data[3] = (
                    abs(bbox["points"]["y2"] - bbox["points"]["y1"])
    ...
    def format_lines(self, lines, image_metadata):
        ...
        for line in lines:
            if line["class"] is not None:
                ...
                data.append(
                    [points, torch.FloatTensor([FRAME_CLASS_DICTIONARY[line["class"]]])]
                )
        return data
        ...
        if not self.preload_images:
            ...
            with torch.no_grad():
                ...
                for i, d in enumerate(data):
                    if self.zipped_images:
                        try:
                            imginfo = zipfile.ZipFile(d["zipfilepath"], "r").open(
                                d["imagefilepath"]
                            )
                            img = transforms.ToTensor()(Image.open(imginfo)) * 255
                            data[i]["image"] = self.resize(img)
                        except KeyError:
                            # Try without .png extension if it fails
                            img_name = d["imagefilepath"].replace(".png", "")
                            imginfo = zipfile.ZipFile(d["zipfilepath"], "r").open(img_name)
                            img = transforms.ToTensor()(Image.open(imginfo)) * 255
                            data[i]["image"] = self.resize(img)
                    else:
                        data[i]["image"] = self.resize(
                            torchvision.io.read_image(d["filepath"])
                        )
                return data
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_v3d", action="store_true", help="Use Labels-v3D.json files (for SoccerNet-v3D dataset)"
    )
